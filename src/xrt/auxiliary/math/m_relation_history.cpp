...
struct m_relation_history
{
	...
	bool filtered_motion_vectors;
	struct m_filter_euro_vec3 position_filter;
	struct m_filter_euro_quat orientation_filter;
};

void
m_relation_history_create(struct m_relation_history **rh_ptr, bool filtered_motion_vectors)
{
	auto ret = std::make_unique<m_relation_history>();

	// @todo ensure these parameters are sane for this usecase
	m_filter_euro_vec3_init(&ret->position_filter, 1, 1.00, 0.00700);

	m_filter_euro_quat_init(&ret->orientation_filter, 1, 1.00, 0.00700);

	ret->filtered_motion_vectors = filtered_motion_vectors;
                                   ...
                                   struct xrt_space_relation *out_relation)
{
	...
	if (rh->filtered_motion_vectors) {
		struct xrt_vec3 previous_position = rh->position_filter.prev_y;
		struct xrt_quat previous_orientation = rh->orientation_filter.prev_y;
		...
		if (in_relation->relation_flags & XRT_SPACE_RELATION_POSITION_VALID_BIT) {
			m_filter_euro_vec3_run(&rh->position_filter, timestamp, &in_relation->pose.position,
			                       &new_position);
		}

		if (in_relation->relation_flags & XRT_SPACE_RELATION_ORIENTATION_VALID_BIT) {
			m_filter_euro_quat_run(&rh->orientation_filter, timestamp, &in_relation->pose.orientation,
			                       &new_orientation);
		}
